...
nav_window_sercurity_mysql = Blueprint('nav_window_sercurity_mysql', __name__)

@nav_window_sercurity_mysql.route("/navigation/retrieveComponentObjectSecurityMySQL", methods=['POST'])
def retrieveComponentObjectSecurityMySQL():
    ...
    if number_of_rows==0:
        ...
        if number_of_rows==0:
            objectSkey = getNextSequenceMySQL('pl_object_def','object_skey','object_skey')
            sql = """\
            ...
            values(%(object_skey)s,%(node_skey)s,%(object_type)s,%(object_comment)s,%(app_skey)s,%(object_name)s,0,user(),now(),user(),now())
            """
...
@nav_window_sercurity_mysql.route("/navigation/updateNodeObjectMySQL", methods=['POST'])
def updateNodeObjectMySQL():
    now = datetime.datetime.now()
    isSuccess = True
    reasonCode = 200
    reasonText = ""
