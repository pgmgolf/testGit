...
nav_route_mssql = Blueprint('nav_route_mssql', __name__)

@nav_route_mssql.route("/navigation/retrieveNodeRouteMSSQL", methods=['POST'])
def retrieveNodeRouteMSSQL():
    try:
        ...
        sql = """\
        EXEC sp_route_maint_sel @a_s_app_name = %(a_s_app_name)s;
        """
        NewValue = 0
        params = {'a_s_app_name': appName}
...
@nav_route_mssql.route("/navigation/addNodeRouteMSSQL", methods=['POST'])
def addNodeRouteMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
        ...
        for node in toNode:
            print(node)
            if fromNodeSkey == node['node_skey']:
                continue
            sql = """\
            ...
            """
            params = {'app_skey': appSkey, 'from_node_skey': fromNodeSkey, 'to_node_skey': node['node_skey']}
            cursor.execute(sql,params)
            (number_of_rows,)=cursor.fetchone()

            if number_of_rows==0:
                sql = """\
                select isnull(max(sort_seq),0) + 1 as max_sort_seq from pn_node_route
                where app_skey = %(app_skey)s and
                ...
                values(%(app_skey)s,%(from_node_skey)s,%(to_node_skey)s,%(sort_seq)s,%(user_id)s,getdate(),%(user_id)s,getdate())
                """
                params = {'app_skey': appSkey,'from_node_skey': fromNodeSkey,'to_node_skey': node['node_skey'],'sort_seq': max_sort_seq,'user_id': userID}
                cursor.execute(sql,params)
                conn.commit()
...
@nav_route_mssql.route("/navigation/updateNodeRouteMSSQL", methods=['POST'])
def updateNodeRouteMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
...
@nav_route_mssql.route("/navigation/updateNodeRouteDragTargetMSSQL", methods=['POST'])
def updateNodeRouteDragTargetMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
...
@nav_route_mssql.route("/navigation/deleteNodeRouteMSSQL", methods=['POST'])
def deleteNodeRouteMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
...
@nav_route_mssql.route("/navigation/retrieveNodeRouteFromNodeMSSQL", methods=['POST'])
def retrieveNodeRouteFromNodeMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
