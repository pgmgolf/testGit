...
episode_diagnose_mysql = Blueprint('episode_diagnose_mysql', __name__)

@episode_diagnose_mysql.route("/episodeDiagnose/retrieveDiagnoseMySQL", methods=['POST'])
def retrieveDiagnoseMySQL():
    try:
...
@episode_diagnose_mysql.route("/episodeDiagnose/insertOrUpdateEpisodeDiagnoseMySQL", methods=['POST'])
def insertOrUpdateEpisodeDiagnoseMySQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
        ...
        if number_of_rows==0:
            diagnoseSkey = getNextSequenceMySQL('ep_diagnose','ep_diagnose','ep_dx_skey')
            sql = """\
            insert into ep_diagnose
            (ep_dx_skey,ep_skey,resource_no,resource_desc,uom_selling,uom_factor,price,qty,vat_rate,include_tax,date_created,user_created,date_changed,user_changed)
            values(%(ep_dx_skey)s,%(ep_skey)s,%(resource_no)s,%(resource_desc)s,%(uom_selling)s,%(uom_factor)s,%(price)s,%(qty)s,%(vat_rate)s,%(include_tax)s,now(),%(user_created)s,now(),%(user_changed)s)
            """
            params = {'ep_dx_skey': diagnoseSkey,'ep_skey': episodeSkey,'resource_no': resourceNo,'resource_desc': resourceDesc
            ...
            where ep_diagnose.ep_skey like %(ep_skey)s and
            ep_diagnose.resource_no like %(resource_no)s
            """
...
@episode_diagnose_mysql.route("/episodeDiagnose/deleteEpisodeDiagnoseMySQL", methods=['POST'])
def deleteEpisodeDiagnoseMySQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
