...
episode_procedure_mysql = Blueprint('episode_procedure_mysql', __name__)

@episode_procedure_mysql.route("/episodeProcedure/retrieveProcedureMySQL", methods=['POST'])
def retrieveProcedureMySQL():
    try:
...
@episode_procedure_mysql.route("/episodeProcedure/insertOrUpdateEpisodeProcedureMySQL", methods=['POST'])
def insertOrUpdateEpisodeProcedureMySQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
        ...
        if number_of_rows==0:
            procedureSkey = getNextSequenceMySQL('ep_procedure','ep_procedure','ep_proc_skey')
            sql = """\
            insert into ep_procedure
            ...
            """
            params = {'ep_proc_skey': procedureSkey,'ep_skey': episodeSkey,'resource_no': resourceNo,'resource_desc': resourceDesc
...
@episode_procedure_mysql.route("/episodeProcedure/deleteEpisodeProcedureMySQL", methods=['POST'])
def deleteEpisodeProcedureMySQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
