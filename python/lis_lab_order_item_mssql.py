...
lis_lab_order_item_mssql = Blueprint('lis_lab_order_item_mssql', __name__)

def insertOrUpdateLabOrderItemMSSQL(conn,dataInsertOrUpdate):
    try:
...
def insertLabOrderItemMSSQL(conn,dataInsert):
    try:
        now = datetime.datetime.now()
        ...
        sql = """\
        EXEC sp_lis_lab_order_item_ins @order_skey = %(order_skey)s,@order_item_skey = %(order_item_skey)s,@x = %(x)s,
                                    @y = %(y)s,@active = %(active)s,@cancel = %(cancel)s,@note = %(note)s,
                                    @rc_date_created = %(rc_date_created)s,@rc_user_created = %(rc_user_created)s
        """
        params = {'order_skey': orderSkey,'order_item_skey': orderItemSkey,'x': x,'y': y,
        'active': active,'cancel': cancel,'note': note,'rc_date_created': rcDateCreated,'rc_user_created': rcUserCreated}
        cursor.execute(sql,params)
...
def updateLabOrderItemMSSQL(conn,dataUpdate):
    try:
        now = datetime.datetime.now()
...
def deleteLabOrderItemMSSQL(conn,dataInsertOrUpdate):
    try:
        now = datetime.datetime.now()
        ...
        sql = """\
        EXEC sp_lis_lab_order_item_del @order_skey = %(order_skey)s,@order_item_skey = %(order_item_skey)s
        """
        params = {'order_skey': orderSkey,'order_item_skey': orderItemSkey
        }
        cursor.execute(sql,params)
