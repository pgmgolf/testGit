...
nav_window_mssql = Blueprint('nav_window_mssql', __name__)

@nav_window_mssql.route("/navigation/retrieveWindowMSSQL", methods=['POST'])
def retrieveWindowMSSQL():
    try:
...
@nav_window_mssql.route("/navigation/addWindowMSSQL", methods=['POST'])
def addWindowMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        ...
        winSkey = getNextSequenceMSSQL('pc_window_def','win_skey','win_skey')
        sql = """\
        ...
        values(%(win_skey)s,%(app_skey)s,%(win_name)s,'O',1,%(window_comment)s,%(user_id)s,getdate(),%(user_id)s,getdate())
        """
...
@nav_window_mssql.route("/navigation/updateWindowMSSQL", methods=['POST'])
def updateWindowMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
...
@nav_window_mssql.route("/navigation/deleteWindowMSSQL", methods=['POST'])
def deleteWindowMSSQL():
    try:
        now = datetime.datetime.now()
        isSuccess = True
        reasonCode = 200
